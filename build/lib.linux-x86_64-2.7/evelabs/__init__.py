try:
  ...
  from Queue import Queue
from evelabs.socket_handler import SocketHandler
import time
import string
import random
import sys
...
class evelabs:
  ...
  def connectMenu(self, devices):
    print("Select the evelabs to connect to:")
  ...
  def autoConnect(self, id = None, interactive = False):
    try:
      res = request.urlopen("http://local.evelabs.io/devices.json").read()
    except:
  ...
  def analogInput(self, pin):
    return self.__send('analogInput',   pin,  1)

  def digitalInput(self, pin):
    return self.__send('digitalInput',   pin,  1)

  def gpioOn(self, pin):
    return self.__send('gpio_on',   pin,  1)

  def gpioOff(self, pin):
    return self.__send('gpio_off',   pin,  1)
  ...
  def __send_or_raise(self, msg, timeout):
    ...
    while True:
      try:
        ...
        if (accepted):
          raise IOError("evelabs timed out awaiting completion of %r" % (msg,))
        raise IOError("evelabs timed out awaiting acceptance of %r" % (msg,))
